...
!function($, window, document){
	...
	FileUploader.prototype  = {
		init:function(){
			this.$element.addClass('layui-hide').after('\
		...
		data:{
			...
			deletedFiles:{},//记录删除的已上传的文件
		},
		errorMsg:function(content){
			layui.use('layer',function(){
				var layer = layui.layer;
				return layer.msg(content, {icon: 2 ,shift: 6 ,time:2000});
			})
		},
			...
			that.uploader.on( 'fileQueued', function(file) {
				layui.use('layer',function(){
					...
					var $tr = $(['<tr id="'+ file.id +'">'
				        ,'<td class="name"><a title="'+file.name+'" class="upload-img-prev-link" href="javascript:;">'+ file.id +'</a></td>'
					...
					reader.onload = function(data){
			 	        ...
			 	        $tr.find('.upload-img-prev-link').on('mouseover', function(){
			 	        	thisTipIndex = layer.tips(img, $(this),{
			 	        		tips: [2, 'rgba(35,38,46,0.5)'],
			 	        		maxWidth: 350,
			 	        		time: 5000,
			 	        	});
			 	        }).on('mouseleave', function(){
			 	        	layer.close(thisTipIndex);
			 	        });
								...
								function(){
									...
									if(that.options.isRealTimeDelete){
										$.ajax({
											...
											data:{picPath:that.data.uploadedFiles[file.id]},
											success:function(data){
												if(data.success){
													that.removeFile($tr,file);
													that.data.deletedFiles[file.id]=file;
												}
		...
		uploadProgress:function(){
			...
			that.uploader.on('uploadProgress', function (file, percentage) {
		    	...
		    	layui.use('element',function(){
		    		...
		    		element.progress('image'+file.id, percentage.toFixed(1) * 100+'%');//设置页面进度条
		    	})
		    });
		},
		successFile:function(file,data){
	    	...
	    	if(data.success){
	    		...
	    		this.data.uploadedFiles[file.id]=data.data;//缓存已上传的文件路径
	    	}else{
		...
		uploadFinished:function(){
			...
			that.uploader.on('uploadFinished', function (file) {
				...
				if(Object.keys(that.data.uploadedFiles).length == length){//总上传数=已上传数
					$('button[lay-filter="faqsubmit"]').click();
					typeof that.options.uploadFinished=='function' &&that.options.uploadFinished(file,that.data);
				}
			});
		}
	}
